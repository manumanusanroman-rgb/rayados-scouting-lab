"""
process_opta_events_multi.py
=============================
Procesa eventos crudos de 8 ligas americanas:
  Liga MX        2024-2025
  MLS            2025
  Brasil Serie A 2025
  Brasil Serie B 2025
  Argentina      2024 (Apertura + Clausura, equiv. temporada europea 24-25)
  Chile          2024
  Colombia       2024
  Ecuador        2025

Acumula stats POR PLAYER_ID. Etiqueta cada jugador con liga y temporada.
Genera: data/player_events_stats.csv con ~2500-3500 jugadores.
"""
...
# Configuracion: ZIP -> (carpeta_liga, temporada_a_usar)
LIGAS_CONFIG = [
    # (zip_filename, root_in_zip, liga_folder, temporada, liga_display)
...
# Zonas (eje X 0-100; 100 = porteria rival)
ZONA_AREA_GRANDE = 83
...
def list_match_files(zf, root_in_zip, liga, temporada):
    """Lista JSONs de partidos de una liga-temporada dentro del ZIP."""
    prefix = f"{root_in_zip}/{liga}/{temporada}/partidos/"
    return [n for n in zf.namelist() if n.startswith(prefix) and n.endswith(".json")]


def process_match(zf, path, stats, liga_display):
    """Procesa un partido y acumula stats por player_id."""
    try:
...
def main():
    print("=" * 70)
    print("PROCESAMIENTO MULTI-LIGA - 8 LIGAS AMERICANAS")
    print("=" * 70)
    ...
    # cache de ZIPs abiertos
    zip_handles = {}

    try:
        for cfg in LIGAS_CONFIG:
            zip_name, root_in_zip, liga, temp, liga_display = cfg
            ...
            match_files = list_match_files(zf, root_in_zip, liga, temp)
            if not match_files:
                print(f"  [SKIP] {liga_display} {temp}: 0 partidos")
                continue

            print(f"\n>>> {liga_display} ({temp}): procesando {len(match_files)} partidos...")
            t_liga = time.time()
            eventos_liga = 0
            for path in match_files:
                n = process_match(zf, path, stats, liga_display)
                eventos_liga += n
            total_eventos += eventos_liga
            total_partidos += len(match_files)
            elapsed = time.time() - t_liga
            print(f"    {eventos_liga:,} eventos en {elapsed:.1f}s ({len(stats)} jugadores acumulados)")
    finally:
        # cerrar ZIPs
    ...
    print(f"\n{'='*70}")
    print(f"PROCESADAS {len(LIGAS_CONFIG)} ligas-temporadas")
    print(f"Total partidos: {total_partidos:,}")
    print(f"Total eventos:  {total_eventos:,}")
    print(f"Total jugadores: {len(stats)}")
